...
async function uploadRequest(path, { file, intakeId, kind, token, companyId, userId }) {
  const headers = {};
  if (token) headers.Authorization = `Bearer ${token}`;
  if (companyId) headers["X-Company-Id"] = companyId;
  if (userId) headers["X-User-Id"] = userId;
...
export const participantProtectionApi = {
  ...
  uploadEventImage: async ({ intakeId, kind, file, token, companyId, userId }) =>
    uploadRequest("/v2/media/event-images", {
      file,
      intakeId,
      kind,
      token,
  ...
  publicPrivacyNotice: async ({ publicToken }) =>
    request(`/v2/public/events/${encodeURIComponent(publicToken)}/privacy-notice`),
  ...
  publicParticipant: async ({ publicToken, participantToken }) =>
    request(
      `/v2/public/events/${encodeURIComponent(publicToken)}/participants/${encodeURIComponent(participantToken)}`
    ),

  publicComplete: async ({ publicToken, participantToken, payload }) =>
    request(
      `/v2/public/events/${encodeURIComponent(publicToken)}/participants/${encodeURIComponent(participantToken)}/complete`,
      {
        method: "PUT",
        body: payload,
      }
