...
from app.constants.roles import ROLES, assignable_roles_for, normalize_role
...
def _enforce_user_limit(db: Session, company_id: str | UUID) -> None:
    company_uuid = UUID(str(company_id))
    license_row = CompanyLicenseService.ensure_license(db, company_uuid)
    usage = CompanyLicenseService.get_usage_summary(db, company_uuid)
...
def _ensure_assignable(caller_role: str, target_role: str) -> None:
    allowed = set(assignable_roles_for(caller_role))
    if target_role not in allowed:
        raise HTTPException(
            status_code=403,
            detail=f"{caller_role} cannot assign role {target_role}",
        )
...
def create_user(
    ...
    _ensure_assignable(caller_role, role)
    ...
    if bool(payload.active):
        _enforce_user_limit(db, target_company_id)
    ...
    setattr(u, "email", str(payload.email).lower())
...
def patch_user(
    ...
    if "role" in payload:
        ...
        _ensure_assignable(caller_role, new_role)
        setattr(u, "role", new_role)

    if "active" in payload and hasattr(u, "active"):
        new_active = bool(payload["active"])
        current_active = bool(getattr(u, "active", True))
        if new_active and not current_active:
            _enforce_user_limit(db, getattr(u, "company_id"))
        setattr(u, "active", new_active)
