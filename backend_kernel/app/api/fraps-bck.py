...
def _fmt_local(dt: Optional[datetime]) -> str:
    """
    Imprime usando la zona horaria del propio timestamp (offset incluido).
    - Si viene naive, asumimos UTC para no romper.
    """
    if not dt:
        return ""
    try:
        if dt.tzinfo is None:
            return dt.strftime("%d/%m/%Y %H:%M:%S")  # type: ignore[attr-defined]
        # Mantén el offset del dispositivo (lo que capturó el navegador)
        return dt.astimezone(dt.tzinfo).strftime("%d/%m/%Y %H:%M:%S")
    except Exception:
        try:
            return str(dt)
        except Exception:
...
def _event_detail(row_type: str, data: Any) -> list[str]:
    ...
    if t in ("injury", "map", "bodymap"):
        kind = _cap(data.get("kind") or data.get("type") or "")
        location = _cap(data.get("location") or data.get("site") or "")
        notes = _cap(data.get("notes") or data.get("note") or "")
        line = "Lesión"
        if kind:
            line = f"{line} - {kind}"
        if location:
            line = f"{line} - Sitio: {location}"
        lines = [line]
        if notes:
            lines.append(f"Notas: {notes}")
        return [l for l in lines if l.strip()]
